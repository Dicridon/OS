...
#define PROCESS1_OFF 0x7400
#define PROCESS2_OFF 0x8200
...
uint32_t write_process(FILE* image_file, char* process_name){
    char buffer[SECTOR];
    char filling[SECTOR] = "";
    int file_size;
    int chunk = SECTOR;
...
uint32_t write_process1(FILE* image_file, char* process_name){
    char buffer[SECTOR];
    char filling[SECTOR] = "";
    int file_size;
    int chunk = SECTOR;
    ...
    fseek(image_file, PROCESS1_OFF, SEEK_SET);
    while(file_size > 0){
	read_size = (chunk <= file_size) ? chunk : file_size;
	fread(buffer, 1, read_size, process);
	fwrite(buffer, 1, read_size, image_file);
...
uint32_t write_process2(FILE* image_file, char* process_name){
    char buffer[SECTOR];
    char filling[SECTOR] = "";
    int file_size;
    int chunk = SECTOR;
    ...
    fseek(image_file, PROCESS2_OFF, SEEK_SET);
    while(file_size > 0){
	read_size = (chunk <= file_size) ? chunk : file_size;
	fread(buffer, 1, read_size, process);
	fwrite(buffer, 1, read_size, image_file);
...
int main(int argc, char *argv[]){
    ...
    else if(argc == 6 && !strcmp(argv[1], "--extended")){
	bootblock = argv[2];
	kernel = argv[3];
	process1 = argv[4];
	process2 = argv[5];
...
//	printf("writing %s\n", process1);
//	write_process(image_file, process1);
//
//	printf("writing %s\n", process2);
//	process_size = write_process(image_file, process2);
//
//	printf("writing %s\n", process3);
//	process_size = write_process(image_file, process3);

	printf("writing %s\n", process1);
	write_process1(image_file, process1);

	printf("writing %s\n", process2);
	process_size = write_process2(image_file, process2);
	
	record_kernel_sectors(image_file, SECTOR, process_size+0x8200);	
