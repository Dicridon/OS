...
void handle_tlb_c(void)
{
    ...
    uint32_t* PTDir = (uint32_t*)current_running->page_dir;
    uint32_t PTDItem = PTDir[badvaddr >> 22];
    uint32_t* PTEntry = (uint32_t*)(*((uint32_t*)(PTDItem & 0xfffff000)));
    uint32_t PTEntryItem = PTEntry[(badvaddr >> 12) & 0x3ff];
	...
	PTDir[(badvaddr>>22)] = ((page_paddr(index) + 0xa0000000)) | 0xc00;
	if(!(PTEntryItem) & 0xc00){
	    index = page_alloc(badvaddr >= 0x300000);
	    PTEntry[(badvaddr>>12) & 0x3ff] = ((page_paddr(index)+0xa0000000)) | 0xc00;
	    if(!(PTBase[VPN] & PE_V)){
		printf(line++, 1, "Memory Allocation %x", current_running->user_tf.cp0_badvaddr);
	
		index = page_alloc(badvaddr >= 0x300000);
		PTBase[VPN] = (page_paddr(index) & 0xfffff000) >> 6 | (PE_V | PE_D);
		uint32_t entry_hi = ((VPN << 12) & 0xffffe000) | (current_running->pid & 0xff);
		tlb_flush(entry_hi);
...
void printsp(uint32_t sp){
    printf(28, 1, "SP IS %x", sp);
}
